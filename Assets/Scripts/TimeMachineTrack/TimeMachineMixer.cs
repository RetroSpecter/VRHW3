...
public class TimeMachineMixer : PlayableBehaviour
{
    ...
    public override void ProcessFrame(Playable playable, FrameData info, object playerData)
    {
        ...
        for (int i = 0; i < inputCount; i++)
        {
            ...
            ScriptPlayable<TimeMachineBehavior> inputPlayable = (ScriptPlayable<TimeMachineBehavior>)playable.GetInput(i);
            TimeMachineBehavior input = inputPlayable.GetBehaviour();
                ...
                if (!input.clipExecuted)
                {
                    switch (input.action)
                    {
                        ...
                        case TimeMachineBehavior.TimeMachineAction.JumpToMarker:
                            ...
                            {
                                //Rewind
                                if (input.action == TimeMachineBehavior.TimeMachineAction.JumpToTime)
                                {
                                    //Jump to time
